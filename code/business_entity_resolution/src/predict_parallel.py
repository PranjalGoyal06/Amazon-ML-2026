...
global_test_target_df = None

def init_worker(blocker, model, thresh, target_df):
    """Initialize global variables for the worker processes."""
    global global_blocker, global_model, global_best_thresh, global_test_target_df
    global_blocker = blocker
    global_model = model
    global_best_thresh = thresh
    global_test_target_df = target_df

def process_chunk(chunk):
    """Worker function to process a chunk of test_source1."""
    # 1. Normalize
    ...
    cands_dict = global_blocker.generate_candidates_for_df(chunk, show_progress=False)
    ...
    if len(chunk_pairs_df) > 0:
        ...
        chunk_target_df = global_test_target_df[global_test_target_df["entity_id"].isin(needed_target_ids)].copy()
        ...
        chunk_features["prob"] = global_model.predict_proba(chunk_features)
        
        matches = chunk_features[chunk_features["prob"] >= global_best_thresh]
...
def main():
    ...
    # 1. Train Model
    model, best_thresh = train_model(base_dir)
    ...
    test_s2 = pd.read_csv(test_s2_path, sep="\t", usecols=["entity_id", "business_name", "business_address", "country"])
    test_s3 = pd.read_csv(test_s3_path, sep="\t", usecols=["entity_id", "business_name", "business_address", "country"])
    ...
    # 4. Process test_source1.tsv in parallel chunks
    ...
    print(f"Processing test_source1.tsv in parallel using {cpu_count()} cores...")
    
    # Read all chunks into a list (100k per chunk)
    chunks = list(pd.read_csv(test_s1_path, sep="\t", chunksize=25000))
    
    # Parallel processing
    with Pool(processes=cpu_count(), initializer=init_worker, initargs=(blocker_test, model, best_thresh, test_target_df)) as pool:
        results = pool.map(process_chunk, chunks)
        
    print("Writing results to disk...")
    for cands_out, matches_out in results:
    ...
    print(f"Done generating predictions on test set in {total_time/60:.2f} minutes.")
